...
static const int DOCNUM = 1000;
static const int DOCSIZE = 1024 * 32;

enum state_t {
    Null = -1, Initing = 0, Running = 1, Destroying = 2,
};
...
int main() {
    ...
    while (true) {
        ...
        case Null:
            assert(
                    s_pending_conf == 0 && s_pending_set == 0
                            && s_pending_get == 0);
...
static void configuration_callback(lcb_t instance, lcb_configuration_t config) {
    ...
    if (s_state == Running && s_pending_set == 0 && s_pending_conf == 0
            && s_pending_get == 0)
        emit_check();
}
...
static void emit_check() {
    for (int i = 0; i < DOCNUM; ++i) {
        ...
        lcb_error_t err = lcb_get(s_inst, NULL, 1, cmds);
        if (err == LCB_SUCCESS) {
            ++s_pending_get;
        } else {
